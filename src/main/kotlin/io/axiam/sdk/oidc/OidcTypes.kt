 ...
 * whole, one level in. A `null` entry falls back to the top-level endpoint of
...
data class OidcConfiguration(
     ...
     *
     * `null` means **"no separate host", not "mTLS unsupported"**: a deployment
     ...
     * topology AXIAM ships. The server omits the key rather than serialising
     * `null`.
     */
